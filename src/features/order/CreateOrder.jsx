...
function CreateOrder() {
  ...
  const priorityPrice= withPriority ? totalCartPrice * 0.2 : 0;   // extra 20%
...
export async function action({request}){
  ...
  const order={
    ...data,
    cart: JSON.parse(data.cart),   //json string into javascript object
    priority: data.priority === 'true'
  }

  const errors= {};
  if(!isValidPhone(order.phone))
    errors.phone= "Please provide us your correct phone no."
  ...
  return redirect(`/order/${newOrder.id}`);    // react-router's function
}
